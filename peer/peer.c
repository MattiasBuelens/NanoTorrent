...
void nanotorrent_peer_init(nanotorrent_torrent_state_t *state) {
	...
	udp_socket_register(socket, state, nanotorrent_peer_handle_message);
	// Store listen port
...
void nanotorrent_peer_send_message(nanotorrent_torrent_state_t *state,
		const uint8_t *buffer, uint16_t buffer_length,
		const nanotorrent_peer_info_t *remote_peer) {
	udp_socket_sendto(&state->peer.peer_socket, buffer, buffer_length,
			&remote_peer->peer_ip, remote_peer->peer_port);
}

void nanotorrent_peer_handle_data_request(nanotorrent_torrent_state_t *state,
		const uint8_t *buffer, uint16_t buffer_length,
		const nanotorrent_peer_info_t *remote_peer) {
	...
	if (!nanotorrent_piece_is_complete(state, request.piece_index)) {
	...
	uint16_t data_length = nanotorrent_peer_write_data_reply(state, &reply_cur,
	...
	nanotorrent_peer_send_message(state, reply_buffer, reply_length,
			remote_peer);
}
...
void nanotorrent_peer_handle_message(struct udp_socket *peer_socket, void *ptr,
	...
	case NANOTRACKER_PEER_DATA_REQUEST:
		nanotorrent_peer_handle_data_request(state, data, datalen,
				&remote_peer);
		break;
...
CC_INLINE void nanotorrent_peer_make_header(
		const nanotorrent_torrent_state_t *state,
...
void nanotorrent_peer_write_close(const nanotorrent_torrent_state_t *state,
	...
	nanotorrent_peer_make_header(state, &message.header,
			NANOTRACKER_PEER_CLOSE);
...
void nanotorrent_peer_write_have(const nanotorrent_torrent_state_t *state,
	...
	nanotorrent_peer_make_header(state, &message.header, NANOTRACKER_PEER_HAVE);
...
void nanotorrent_peer_write_data_request(
	...
	nanotorrent_peer_data_request_t request;
	nanotorrent_peer_make_header(state, &request.header,
...
uint16_t nanotorrent_peer_write_data_reply(
	...
	nanotorrent_peer_make_header(state, &reply.header,
	...
	uint16_t data_length = nanotorrent_piece_read(state, piece_index,
			data_start, *cur, buffer_size - header_length);
	if (data_length < 0) {
		return -1;
	}
	*cur += data_length;
